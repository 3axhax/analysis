...
interface AgesFilters {
  name?: string;
  translationRu?: string;
  translationEn?: string;
}

function isValidTranslationsFilters(obj: unknown): obj is AgesFilters {
...
export class GetAgesListQueryDto {
  ...
  @Transform(({ value }: TransformFnParams): AgesFilters => {
...
export class AddNewAgesQueryDto {
  @IsString()
  name: string;
...
export class EditAgesQueryDto extends AddNewAgesQueryDto {
  @Transform(({ value }: TransformFnParams) => {
    const num = value ? parseInt(value as string, 10) : 1;
    return isNaN(num) || num < 1 ? 1 : num;
  })
